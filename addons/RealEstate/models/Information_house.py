...
class Product(models.Model):
    ...
    additional_images_2 = fields.Many2many(
        ...
        # groups='RealEstate.group_people_dauchu,RealEstate.group_people_chunha,RealEstate.group_people_daukhach',
        domain=[('mimetype', 'ilike', 'image')],
    )
    ...
    #     'ir.attachment', 
    #     string="Images",
    #     help="Upload multiple images for the product", groups='RealEstate.group_people_dauchu')
    ...
    def _compute_is_approve(self):
        for record in self:
            if record.env.user.has_group('RealEstate.group_people_dauchu'):
                record.isReadonly = False
            else:
                record.isReadonly = True

            if record.env.user in record.follower_2_ids.create_uid or record.env.user.has_group('RealEstate.group_people_dauchu') or record.env.user.has_group('RealEstate.group_people_chunha'):
    ...
    def domain(self):
        ...
        if self.env.user.has_group('RealEstate.group_people_dauchu') or self.env.user.has_group('RealEstate.group_people_chunha'):
            return []
    ...
    def action_create_sale_product(self):
        ...
        res.write({'name': self.name,'isDuplicate':True,'product_parent':self.id})
